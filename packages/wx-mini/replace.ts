import { options as sdkOptions, ReplaceHandler, setTraceId, subscribeEvent, transportData, triggerHandlers } from '../core/index'
import { WxAppEvents, WxPageEvents, WxRouteEvents, WxEvents, EVENTTYPES, HTTPTYPE, voidFun } from '../shared/index'
import { getTimestamp, replaceOld, throttle, getFlag, isEmptyObject, variableTypeDetection, getCurrentRoute } from '../utils/index'
import { HandleWxAppEvents, HandleWxPageEvents } from './handleWxEvents'
import { MITOHttp, EMethods } from '../types/index'
  ...
  if (App) {
    ...
    App = function (appOptions: WechatMiniprogram.App.Option) {
      ...
      methods.forEach((method) => {
        ...
        addReplaceHandler({
          callback: (data) => (HandleWxAppEvents as any)[method.replace('AppOn', 'on')](data),
          type: method
        })
        replaceOld(
          appOptions,
